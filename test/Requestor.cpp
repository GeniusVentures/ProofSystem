...
// Constructor
Requestor::Requestor(size_t num_nodes, size_t num_blocks_per_node)
    : num_nodes(num_nodes), num_blocks_per_node(num_blocks_per_node) {
    // Initialize the generator point (replace with the actual generator point if known)
    generator = typename pallas::template g1_type<nil::crypto3::algebra::curves::coordinates::affine>::value_type(1, 2); // Example coordinates for the generator
    setupNodes();
}
...
void Requestor::setupNodes() {
    ...
    base_nonce = typename pallas::base_field_type::value_type(dis(gen));
    for (size_t i = 0; i < num_nodes; ++i) {
        std::uint64_t random_number = dis(gen); // Generate a random number
...
void Requestor::generateNoncesForNode(size_t node_number) {
    ...
    for (size_t block_index = 0; block_index < num_blocks_per_node; ++block_index) {
        ...
        auto positive_nonce = base_nonce + random_numbers[node_number];
        auto negative_nonce = base_nonce - random_numbers[node_number];
...
typename pallas::template g1_type<nil::crypto3::algebra::curves::coordinates::affine>::value_type Requestor::getTotalRandomSum() const {
    ...
    typename pallas::template g1_type<nil::crypto3::algebra::curves::coordinates::affine>::value_type total_sum = typename pallas::template g1_type<nil::crypto3::algebra::curves::coordinates::affine>::value_type(); // Identity point

    for (const auto& num : random_numbers) {
        // Accumulate the total random sum as G1 points
        total_sum = total_sum + (generator * num); // Convert each number to a curve point using the generator
    }
...
typename pallas::template g1_type<nil::crypto3::algebra::curves::coordinates::affine>::value_type Requestor::getFinalAggregate() const {
    ...
    typename pallas::template g1_type<nil::crypto3::algebra::curves::coordinates::affine>::value_type aggregate = typename pallas::template g1_type<nil::crypto3::algebra::curves::coordinates::affine>::value_type();

    for (size_t i = 0; i < num_nodes; ++i) {
        // Aggregate Nonce A and Nonce B values in the context of the curve
        aggregate = aggregate + nonces_a[i] + nonces_b[i];
    }
