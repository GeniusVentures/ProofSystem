...
TEST( ElGamalKeyGeneratorTest, Initialization )
{
    ...
    for ( size_t i = 0; i < 10; i++ )
    {
        try
        {
            KeyGenerator::ElGamal{ ElGamal::CreateGeneratorParams() };
        }
        catch ( ... )
        {
...
{
    ElGamal             key_generator( 0xb22e83584f11aa1ce949bd0daff1f976da072c60e49fdd3dc40dcb28fd9f1a62_cppui256 );
    ...
    ElGamal::PublicKey pubkey( pubkey_expected_value );
...
TEST( ElGamalKeyGeneratorTest, EncryptionDecryptionNum )
{
    ...
    auto cypher = ElGamal::EncryptData( key_generator.GetPublicKey(), my_value );

    auto result = ElGamal::DecryptData<cpp_int>( key_generator.GetPrivateKey(), cypher );
...
TEST( ElGamalKeyGeneratorTest, EncryptionDecryptionShortVect )
{
    ...
    auto cypher = ElGamal::EncryptData( key_generator.GetPublicKey(), my_vect );

    std::vector<uint8_t> new_vect = ElGamal::DecryptData<std::vector<uint8_t>>( key_generator.GetPrivateKey(), cypher );
...
TEST( ElGamalKeyGeneratorTest, MultiplicativeHomomorphism )
{
    ...
    auto cypher_balance = ElGamal::EncryptData( key_generator.GetPublicKey(), original_balance );
    auto cypher_twice   = ElGamal::EncryptData( key_generator.GetPublicKey(), twice );
    ...
    auto result = ElGamal::DecryptData<cpp_int>( key_generator.GetPrivateKey(), cypher_total );
...
TEST( ElGamalKeyGeneratorTest, AdditiveHomomorphism )
{
    ...
    auto cypher_50_exp = ElGamal::EncryptDataAdditive( key_generator.GetPublicKey(), original_balance );
    auto cypher_2_exp  = ElGamal::EncryptDataAdditive( key_generator.GetPublicKey(), two );
